...
def test_bad_cla(cmd):
    ...
    with pytest.raises(errors.ClaNotSupportedError):
...
def test_bad_ins(cmd):
    ...
    with pytest.raises(errors.InsNotSupportedError):
        raise DeviceException(error_code=sw)
...
def test_wrong_p1p2(cmd):
    ...
    with pytest.raises(errors.WrongP1P2Error):
        raise DeviceException(error_code=sw)
...
def test_wrong_data_length(cmd):
    ...
    with pytest.raises(errors.WrongDataLengthError):
        raise DeviceException(error_code=sw)